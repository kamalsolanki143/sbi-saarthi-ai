'use client'
import { motion } from 'framer-motion'
import { useRouter } from 'next/navigation'
import TopBar from '@/components/shared/TopBar'
import ActionPreview from '@/components/ActionPreview/ActionPreview'
import { useAgentStore } from '@/store/useAgentStore'
import MPINVerification from '@/components/MPINVerification/MPINVerification'

const pageVariants = {
  initial: { opacity: 0, y: 16 },
  animate: { opacity: 1, y: 0, transition: { duration: 0.3, ease: [0.22, 1, 0.36, 1] } },
}

export default function ActionPreviewPage() {
  const { pendingAction, awaitingMPIN, setAwaitingMPIN, setPendingAction, activeAgent } = useAgentStore()
  const router = useRouter()

  if (!pendingAction) {
    return (
      <>
        <TopBar title="Action Preview" />
        <motion.div
          variants={pageVariants}
          initial="initial"
          animate="animate"
          className="flex-1 flex items-center justify-center"
        >
          <p className="text-text-muted text-sm">No pending action. Go to Voice Banking to start.</p>
        </motion.div>
      </>
    )
  }

  return (
    <>
      <TopBar title="Action Preview" />
      <motion.div
        variants={pageVariants}
        initial="initial"
        animate="animate"
        className="flex-1 overflow-y-auto p-6 max-w-lg mx-auto"
      >
        <ActionPreview
          action={pendingAction}
          agent={(activeAgent as any) || 'margdarshan'}
          onAuthorize={() => setAwaitingMPIN(true)}
          onDecline={() => { setPendingAction(null); router.push('/dashboard') }}
        />
        {awaitingMPIN && (
          <MPINVerification
            actionLabel={pendingAction.display_label}
            onSuccess={() => {
              setAwaitingMPIN(false)
              setPendingAction(null)
              router.push('/dashboard')
            }}
            onCancel={() => setAwaitingMPIN(false)}
          />
        )}
      </motion.div>
    </>
  )
}
